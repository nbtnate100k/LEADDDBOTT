...
- Talk to the **same** bot the token belongs to in `.env`. Opening a different bot chat will not use this code. The **📣 Channel** keyboard button defaults to **[HomerLeads](https://t.me/homerleadss)** — override with `PUBLIC_CHANNEL_URL` / `TELEGRAM_CHANNEL_USERNAME` if needed.
...
2. **New project → Deploy from GitHub** (or upload this repo). Railway sets **`PORT`**; the app binds **`0.0.0.0`** on that port using **Waitress** (not Flask’s dev server). **`railway.toml`** points health checks at **`/health`**. If **`TELEGRAM_BOT_TOKEN`** is missing, the HTTP server still starts so the deploy can go green, but the bot will not answer in Telegram until you add the token and redeploy. Locally (no **`PORT`**), a missing token still exits immediately.
