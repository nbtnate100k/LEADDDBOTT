...
# Optional legacy suffix after gender (old HTML syncs); stripped from display.
META_AGE_SUFFIX = "|§a§"
...
def export_full_stock_tsv() -> str:
    """Every raw line in both piles: pile, BIN, full line (tab-separated)."""
    rows = ["pile\tbin6\tline"]
    for tier, pile_label in (("first", "firsthand"), ("second", "secondhand")):
        pile = load_leads(tier)
        for b in sorted(pile.keys()):
            for line in pile[b]:
                rows.append(f"{pile_label}\t{b}\t{line}")
    return "\n".join(rows)


def export_stock_lines_for_brand(brand: str) -> tuple[str, int]:
    """Export all raw lines for catalog BINs of this card network that have stock."""
    from catalog_store import load_catalog

    catbins = [
        str(x).strip()
        for x in load_catalog().get("bins", [])
        if len(str(x).strip()) == 6 and str(x).strip().isdigit()
    ]
    want = [b for b in catbins if card_brand_from_bin6(b) == brand]
    rows: list[str] = ["pile\tbin6\tline"]
    n = 0
    for tier, pile_label in (("first", "firsthand"), ("second", "secondhand")):
        pile = load_leads(tier)
        for b in sorted(want):
            for line in pile.get(b, []):
                rows.append(f"{pile_label}\t{b}\t{line}")
                n += 1
    body = "\n".join(rows) if n else "pile\tbin6\tline\n"
    return body, n
...
def pop_n_random_filtered(
    ...
    *,
    ...
    """Random draw from tier pile, optionally filtered by state, BIN, city, bank, brand, ZIP, gender."""
    t = norm_stock_tier(tier)
    if n < 1:
        return None
