...
from bin_leads_store import (
    ...
    bin_line_counts,
    card_brand_from_bin6,
    clear_bin_leads,
    count_matching_lines,
    export_full_stock_tsv,
    export_stock_lines_for_brand,
...
BOT_BUILD = "shop-v26"
# Admin private chat: /@username 50 → credit $50 to that @username (also /credit @user 50).
_ADMIN_CREDIT_BY_AT = re.compile(
    r"^/\s*@([A-Za-z][A-Za-z0-9_]{4,31})\s+([\d,.]+)\s*$"
)
...
# Notify admins (DM + file) when a single checkout sells this many lines or more.
ADMIN_BULK_PURCHASE_NOTIFY_MIN = 50

DEFAULT_PUBLIC_CHANNEL_URL = "https://t.me/lynchemleads"
SUPPORT_CONTACT_URL = "https://t.me/LYNCHEMSUPPORT"
FREE_LEADS_PER_WEEK = 2
...
def _free_week_key() -> str:
    return datetime.now(timezone.utc).strftime("%G-W%V")


def _ensure_free_week_synced(user_id: int) -> None:
    users = _load_users()
    uid = str(user_id)
    e = dict(users[uid])
    wk = _free_week_key()
    if e.get("free_week") != wk:
        e["free_week"] = wk
        e["free_used"] = 0
        users[uid] = {**users.get(uid, {}), **e}
        _save_users(users)


def free_leads_remaining(user_id: int) -> int:
    ensure_user(user_id)
    _ensure_free_week_synced(user_id)
    used = int(_load_users()[str(user_id)].get("free_used", 0))
    return max(0, FREE_LEADS_PER_WEEK - used)
...
def _username_lookup_key(raw: str) -> str:
    s = str(raw or "").strip()
    if s.startswith("@"):
        s = s[1:]
    return s.lower()
...
def find_user_ids_by_username(username_raw: str) -> list[int]:
    """Telegram @usernames are matched case-insensitively against users.json."""
    key = _username_lookup_key(username_raw)
    ...
    for uid_s, entry in users.items():
        if not str(uid_s).isdigit():
            continue
        stored = _username_lookup_key(str(entry.get("username") or ""))
        if stored == key:
            out.append(int(uid_s))
    return sorted(set(out))


def admin_credit_usd(user_id: int, amount_usd: float) -> tuple[float, float]:
    """Add funds to balance (and total_deposits). Returns (previous, new) balance."""
    if amount_usd <= 0:
    ...
    prev = float(entry.get("balance", 0.0))
    new_b = round(prev + amount_usd, 2)
    entry["balance"] = new_b
    entry["total_deposits"] = round(
        float(entry.get("total_deposits", 0.0)) + amount_usd, 2
    )
...
def _normalize_cart_entries(raw) -> list[dict]:
    ...
    for it in raw or []:
        ...
        if kind == "bin":
            ...
            br = _norm_filter_brand(it.get("brand"))
            zp = (
                norm_zip_filter_value(it.get("zip"))
                if it.get("zip")
                else None
            )
            gen = (str(it.get("gender") or "").strip().lower() or None)
            if gen and gen not in ("male", "female"):
                gen = None
            bn = _norm_bin_input(str(it.get("bin", ""))) if it.get("bin") else None
            if not any([st, ct, br, zp, gen, bn]):
                continue
            out.append(
                {
                    "kind": "filter",
...
def _filter_cart_entry_same(a: dict, b: dict) -> bool:
    return (
        (a.get("state") or "") == (b.get("state") or "")
        and (a.get("city") or "") == (b.get("city") or "")
        and (a.get("brand") or "") == (b.get("brand") or "")
        and (a.get("zip") or "") == (b.get("zip") or "")
        and (a.get("gender") or "") == (b.get("gender") or "")
...
def add_to_cart_filter(user_id: int, tier: str, qty: int, flt: dict) -> None:
    ...
    zp = norm_zip_filter_value(flt.get("zip")) if flt.get("zip") else None
    gen = (str(flt.get("gender") or "").strip().lower() or None)
    if gen and gen not in ("male", "female"):
        gen = None
...
BTN_CHANNEL = "📣 Channel"
BTN_SUPPORT = "💬 Support"
BTN_FREE_LEADS = "🎁 Free leads (2/week)"
...
def reply_main_menu_markup(user_id: int | None) -> ReplyKeyboardMarkup:
    ...
    # Plain text button — opens channel URL via handler (URL keyboard buttons can fail on some clients).
    rows.append([KeyboardButton(BTN_CHANNEL), KeyboardButton(BTN_SUPPORT)])
    rows.append([KeyboardButton(BTN_FREE_LEADS)])
...
def admin_panel_text() -> str:
    return (
        ...
        "• <b>Stock summary</b> — short counts in chat + <b>full_stock.tsv</b> "
        "(every raw line in both piles: pile, BIN, line)\n"
        ...
        "• <b>User balances</b> — export every <code>users.json</code> balance (+ usernames if known)\n"
        "• <b>Quick user lookup</b> — <code>/user TELEGRAM_ID</code> balance & totals "
        "(one user, no file)\n"
        "• <b>Credit by @username</b> — send <code>/@username 50</code> (adds <b>$50</b> to that user) "
        "or <code>/credit @username 50</code>\n"
        "• <b>Bulk sales</b> — purchases of "
        f"<b>{ADMIN_BULK_PURCHASE_NOTIFY_MIN}+</b> lines DM admins the full raw file + stock left\n\n"
        "Commands: <code>/user</code> · <code>/credit</code> · <code>/addbin</code> · "
        "<code>/clearbin</code> · <code>/cancel</code>"
    )
...
def admin_menu_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton(
                    "👥 User balances", callback_data="adm_ub"
                )
            ],
...
async def _scheduled_balance_broadcast(context: ContextTypes.DEFAULT_TYPE) -> None:
    """Send each known user their current balance on a timer."""
    ...
    for cid in ids:
        try:
            await bot.send_message(
                chat_id=cid,
                text="<i>Scheduled balance update</i>\n\n"
                + account_balance_text(cid),
                parse_mode=ParseMode.HTML,
            )
            ok += 1
        except TelegramError:
            fail += 1
            logger.debug("Balance reminder failed chat_id=%s", cid, exc_info=True)
        await asyncio.sleep(0.05)
    logger.info(
        "Scheduled balance reminder: delivered=%s failed=%s recipients=%s",
        ok,
        fail,
        len(ids),
    )
        ...
        if len(text) <= 3800:
            ...
            await bot.send_document(
                ...
                caption=(
                    ...
                    "In-bot: /stock → tap Visa / MC / Amex / Discover for the same BIN lists."
                ),
            )
        return True, ""
...
def buy_filters_prompt_keyboard(qty: int | None = None) -> InlineKeyboardMarkup:
    """Embed qty in Add filters callback so it still works if user_data was lost (restart / multi-worker)."""
    add_cb = f"buy_f_add:{int(qty)}" if qty is not None and int(qty) >= 1 else "buy_f_add"
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("Add filters", callback_data=add_cb)],
...
def stock_by_brand_menu_markup() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("Visa", callback_data="stk:v:visa"),
                InlineKeyboardButton("Mastercard", callback_data="stk:v:mastercard"),
            ],
            [
                InlineKeyboardButton("Discover", callback_data="stk:v:discover"),
                InlineKeyboardButton("Amex", callback_data="stk:v:amex"),
            ],
            [InlineKeyboardButton("⬅️ Close", callback_data="stk:x")],
        ]
    )


def format_stock_brand_detail_html(brand: str) -> str:
    catbins = [
        str(b).strip() for b in load_catalog().get("bins", []) if str(b).strip()
    ]
    cf = bin_line_counts("first")
    cs = bin_line_counts("second")
    rows: list[str] = []
    for b in sorted(catbins):
        if len(b) != 6 or not b.isdigit():
            continue
        if card_brand_from_bin6(b) != brand:
            continue
        n1, n2 = cf.get(b, 0), cs.get(b, 0)
        if n1 + n2 < 1:
            continue
        rows.append(
            f"<code>{escape(b)}</code>  ×{n1 + n2}  (1st {n1} · 2nd {n2})"
        )
    title = _FILTER_BRAND_LABELS.get(brand, brand)
    if not rows:
        return (
            f"🏷️ <b>{escape(title)}</b>\n\n"
            "<i>No catalog BINs in stock for this network.</i>"
        )
    return (
        f"🏷️ <b>{escape(title)}</b> — catalog BINs available now\n\n" + "\n".join(rows)
    )
...
def _has_any_buy_filter(flt: dict | None) -> bool:
    if not flt:
        return False
    if flt.get("bin") and _norm_bin_input(str(flt.get("bin", ""))):
        return True
    return any(flt.get(k) for k in ("state", "city", "zip", "brand", "gender"))
...
def _buy_filters_summary_html(flt: dict | None) -> str:
    ...
    if flt.get("bin"):
        nb = _norm_bin_input(str(flt["bin"]))
        if nb:
            parts.append(f"BIN: <code>{escape(nb)}</code>")
    if flt.get("city"):
    ...
    if flt.get("gender") in ("male", "female"):
        parts.append(
            "Gender: <b>"
            + ("♂ Male" if flt["gender"] == "male" else "♀ Female")
            + "</b>"
        )
    if not parts:
        return "Current filters: <i>(none)</i>"
...
def buy_filters_grid_text(qty: int, flt: dict | None) -> str:
    return (
        ...
        "<i>Tap <b>Brand</b>, <b>ZIP</b>, <b>State</b>, <b>City</b>, <b>BIN</b>, or <b>Gender</b> (from stock). "
        "Then <b>Done</b> for random checkout, or <b>Add filtered to cart</b>.</i>"
    )
...
def buy_filters_grid_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("⚧️ Gender", callback_data="bf_gender")],
...
def buy_flow_help_text() -> str:
    return (
        ...
        "2️⃣ Optionally add filters — <b>Brand</b>, <b>ZIP</b>, <b>State</b>, <b>City</b>, "
        "<b>BIN</b>, <b>Gender</b>. "
        "ZIP = 9th <code>|</code> field when present.\n"
        "3️⃣ Choose <b>firsthand</b> or <b>secondhand</b> tier and confirm.\n"
        "4️⃣ Pay with your <b>credit balance</b> (top up under 💰).\n\n"
        "<i>Then <b>Done</b> or <b>Add filtered to cart</b>. "
        f"Weekly: <b>{FREE_LEADS_PER_WEEK}</b> free leads via 🎁. Advanced: 📋 Browse BINs.</i>"
    )
...
def format_cart_screen(user_id: int) -> str:
    ...
    for i, it in enumerate(cart, 1):
        ...
        if it.get("kind") == "filter":
            ...
            if it.get("gender") in ("male", "female"):
                bits.append("♂" if it["gender"] == "male" else "♀")
...
async def _notify_admins_large_purchase(
    ...
    if len(pairs) < ADMIN_BULK_PURCHASE_NOTIFY_MIN:
        return
    aids = get_admin_ids()
    if not aids:
        return
    n_first = total_line_count("first")
    n_second = total_line_count("second")
    body = "\n".join(f"[{i + 1}] {line}" for i, (_, line) in enumerate(pairs))
    raw = body.encode("utf-8")
    uid = from_user.id
    un = f"@{from_user.username}" if from_user.username else "—"
    fn = escape(from_user.full_name or "—")
    cap = (
        f"📦 <b>Bulk sale</b> (≥{ADMIN_BULK_PURCHASE_NOTIFY_MIN} lines)\n"
        f"Buyer: <code>{uid}</code> {escape(un)} — {fn}\n"
        f"<b>{escape(title)}</b> · <b>${total_price:.2f}</b> · <b>{len(pairs)}</b> lines\n"
        f"<b>Stock left</b> — 1st: <b>{n_first}</b> · 2nd: <b>{n_second}</b> · "
        f"total: <b>{n_first + n_second}</b>\n"
        "<i>Full raw lines as stored (incl. sync tags).</i>"
    )
    fname = f"bulk_{uid}_{len(pairs)}lines.txt"
    for aid in aids:
        if aid == uid:
            continue
        try:
            ...
            logger.warning("Bulk-sale admin notify → %s: %s", aid, e)
...
async def deliver_purchased_bulk(
    ...
    if query.from_user and len(pairs) >= ADMIN_BULK_PURCHASE_NOTIFY_MIN:
        await _notify_admins_large_purchase(
            context, query.from_user, pairs, total_price, title
        )


async def show_admin_user_balances(
    query, context: ContextTypes.DEFAULT_TYPE
) -> None:
    users = _load_users()
    ...
    for uid_s, entry in users.items():
        ...
        fn = str(entry.get("full_name") or "").strip()
        rows.append((uid, bal, un, fn))
    rows.sort(key=lambda x: x[0])
    lines_txt = ["user_id\tbalance_usd\tusername\tfull_name"]
    for uid, bal, un, fn in rows:
        lines_txt.append(f"{uid}\t{bal:.2f}\t{un}\t{fn}")
    body = "\n".join(lines_txt) if len(lines_txt) > 1 else "user_id\tbalance_usd\tusername\tfull_name\n"
    payload = body.encode("utf-8")
    cap = (
        f"👥 <b>{len(rows)}</b> users · total balance in system <b>${total_bal:.2f}</b>"
        if rows
        else "👥 <b>0</b> users"
    )
    await context.bot.send_document(
        chat_id=query.message.chat_id,
        document=InputFile(io.BytesIO(payload), filename="user_balances.tsv"),
        caption=cap,
        parse_mode=ParseMode.HTML,
    )
    await query.edit_message_text(
        "👥 <b>User balances</b>\n\n"
        f"Users: <b>{len(rows)}</b>\n"
        f"Sum of balances: <b>${total_bal:.2f}</b>\n\n"
        "<i>TSV attached: <code>user_id</code>, balance, username, name "
        "(name/username filled when the bot has stored them).</i>",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton("⬅️ Admin", callback_data="adm")]]
        ),
...
def format_admin_user_lookup_html(target_id: int) -> str:
    """Load one user from storage for admin view (does not create a new row)."""
    users = _load_users()
    uid_s = str(target_id)
    if uid_s not in users:
        return (
            "👤 <b>Admin · User lookup</b>\n\n"
            f"<i>No record for Telegram ID <code>{target_id}</code> — "
            "they may never have opened the bot in private chat.</i>"
        )
    raw = dict(users.get(uid_s, {}) or {})
    merged = {**_USER_DEFAULTS, **raw}
    name = escape(str(raw.get("full_name") or "—"))
    un_raw = str(raw.get("username") or "").strip()
    un_disp = f"@{escape(un_raw)}" if un_raw else "—"
    bal = float(merged.get("balance") or 0.0)
    dep = float(merged.get("total_deposits") or 0.0)
    spent = float(merged.get("total_spent") or 0.0)
    vip = bool(merged.get("vip"))
    status = escape(str(merged.get("status", "active")))
    cart = merged.get("cart") or []
    cart_n = len(cart) if isinstance(cart, list) else 0
    return (
        "👤 <b>Admin · User lookup</b>\n\n"
        ...
        f"Total deposits: <code>${dep:.2f}</code>\n"
        f"Total spent: <code>${spent:.2f}</code>\n"
        f"VIP: <b>{'yes' if vip else 'no'}</b>\n"
        f"Status: {status}\n"
        f"Cart lines: <b>{cart_n}</b>"
    )
...
def _reset_session_for_start(context: ContextTypes.DEFAULT_TYPE) -> None:
    """Clear purchase flow state when user sends /start (same idea as inline Home)."""
...
async def reply_open_filters_purchase(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    """Reply-keyboard /filter: show inline filter grid when quantity is set, else quantity picker."""
    ...
    if qty < 1:
        await update.message.reply_text(
            "<b>Filters</b>\n\n"
            "Pick <b>how many leads</b> first (🛒 Buy Leads), then use "
            "<b>Add filters</b> — or choose a quantity below.",
            parse_mode=ParseMode.HTML,
...
async def filter_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_user or not update.message:
        return
    await reply_open_filters_purchase(update, context)
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    uid = update.effective_user.id
    _reset_session_for_start(context)
    text = welcome_text(uid)
    markup = reply_main_menu_markup(uid)
...
async def stock_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    ensure_user(update.effective_user.id)
    await update.message.reply_text(
        "<b>Stock by card brand</b>\n\n"
        "<i>Tap a network — the bot sends a file with <b>every stocked line</b> "
        "for that brand (not just BIN totals).</i>",
        parse_mode=ParseMode.HTML,
        reply_markup=stock_by_brand_menu_markup(),
    )
...
async def request_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "<code>male 200</code> (or <code>female</code>) — gender-filtered leads.\n\n"
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if uid in get_admin_ids():
        raw_line = (update.message.text or "").strip()
        m_credit = _ADMIN_CREDIT_BY_AT.match(raw_line)
        if m_credit and not context.user_data.get("awaiting_admin_paste"):
            amt_part = m_credit.group(2).replace(",", "")
            try:
                cred_amt = float(amt_part)
            except ValueError:
                await update.message.reply_text(
                    "Bad amount. Example: <code>/@Xeeznk 50</code>",
                    parse_mode=ParseMode.HTML,
                )
                return
            await _admin_username_credit_reply(
                update, context, m_credit.group(1), cred_amt
            )
            return
    ...
    if context.user_data.get("awaiting_cart_filter_qty"):
        ...
        filt_cb = f"buy_f_add:{qty_buy}" if qty_buy >= 1 else "buy_f_add"
        await update.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    ...
                    [InlineKeyboardButton("⬅️ Filters", callback_data=filt_cb)],
                ]
            ),
        )
        return
    ...
    if context.user_data.get("awaiting_buy_qty"):
        ...
        await update.message.reply_text(
            ...
            parse_mode=ParseMode.HTML,
            reply_markup=buy_filters_prompt_keyboard(q),
        )
        return
    ...
    if raw_txt == BTN_CHANNEL:
        ...
        return
    if raw_txt == BTN_SUPPORT:
        ensure_user(uid)
        await update.message.reply_text(
            "💬 <b>Support</b>\n\n"
            "Contact: @LYNCHEMSUPPORT\n"
            f"{escape(SUPPORT_CONTACT_URL)}",
            parse_mode=ParseMode.HTML,
            reply_markup=reply_main_menu_markup(uid),
        )
        return
    if raw_txt == BTN_FREE_LEADS:
        ensure_user(uid)
        rem = free_leads_remaining(uid)
        wk = _free_week_key()
        used = FREE_LEADS_PER_WEEK - rem
        base = (
            f"🎁 <b>Free weekly leads</b>\n\n"
            f"Week <code>{escape(wk)}</code> (UTC): "
            f"<b>{used}</b> / <b>{FREE_LEADS_PER_WEEK}</b> claimed.\n\n"
        )
        if rem < 1:
            await update.message.reply_text(
                base
                + "<i>No free leads left this week. Next week you get another "
                f"<b>{FREE_LEADS_PER_WEEK}</b>. Paid leads: 💰 Top-up + 🛒 Buy.</i>",
                parse_mode=ParseMode.HTML,
                reply_markup=reply_main_menu_markup(uid),
            )
            return
        await update.message.reply_text(
            base
            + f"<i>Claim <b>{FREE_LEADS_PER_WEEK}</b> random lines once — no charge.</i>",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(
                [
                    [
...
async def user_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Admin: /user TELEGRAM_ID — show saved balance without opening the balances export."""
    if not update.message or not update.effective_user:
        return
    if update.effective_user.id not in get_admin_ids():
        await update.message.reply_text("⛔ Admin only.")
    ...
    if len(args) != 1:
        await update.message.reply_text(
            "<b>Usage:</b> <code>/user TELEGRAM_ID</code>\n\n"
            "Example: <code>/user 50</code>\n"
            "<i>Reads the saved wallet from <code>users.json</code>.</i>",
            parse_mode=ParseMode.HTML,
        )
        return
    try:
        ...
        await update.message.reply_text(
            "Invalid user id (need a numeric Telegram ID). Example: "
            "<code>/user 123456789</code>",
            parse_mode=ParseMode.HTML,
        )
        return
    await update.message.reply_text(
        format_admin_user_lookup_html(tid),
        parse_mode=ParseMode.HTML,
    )


async def credit_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Admin: /credit @username AMOUNT — same as sending /@username AMOUNT."""
    if not update.message or not update.effective_user:
        return
    if update.effective_user.id not in get_admin_ids():
        await update.message.reply_text("⛔ Admin only.")
    ...
    if len(args) != 2:
        await update.message.reply_text(
            "<b>Usage:</b> <code>/credit @USERNAME AMOUNT</code>\n\n"
            "Same as typing: <code>/@USERNAME AMOUNT</code>\n"
            "Example: <code>/credit @Xeeznk 50</code>\n\n"
            "<i>The user must message the bot once so we saved their @username "
            "in <code>users.json</code>.</i>",
            parse_mode=ParseMode.HTML,
        )
        return
    amt_s = str(args[1]).replace(",", "").strip()
    try:
        amt = float(amt_s)
    except ValueError:
        await update.message.reply_text(
            "Invalid amount. Example: <code>/credit @Xeeznk 50</code>",
            parse_mode=ParseMode.HTML,
        )
        return
    un = str(args[0]).strip().lstrip("@")
    await _admin_username_credit_reply(update, context, un, amt)


async def _admin_username_credit_reply(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
    username_no_at: str,
    amount: float,
    ...
    if not msg:
        return
    un = username_no_at.strip().lstrip("@")
    if not re.fullmatch(r"[A-Za-z][A-Za-z0-9_]{4,31}", un):
        await msg.reply_text(
            "Invalid Telegram username. Use 5–32 chars: letter, then letters/digits/_.\n"
            "Example: <code>/@Xeeznk 50</code>",
            parse_mode=ParseMode.HTML,
        )
        return
    if amount <= 0 or amount > 1_000_000:
        await msg.reply_text(
            "Amount must be &gt; 0 and ≤ <code>1,000,000</code> USD.",
            parse_mode=ParseMode.HTML,
        )
        return
    ids = find_user_ids_by_username(un)
    if not ids:
        await msg.reply_text(
            f"No saved user with <code>@{escape(un.lower())}</code>. "
            "They need to open the bot in private chat at least once.",
            parse_mode=ParseMode.HTML,
        )
        return
    if len(ids) > 1:
        ...
        await msg.reply_text(
            f"Multiple accounts match <code>@{escape(un)}</code> — stale usernames:\n{lines}\n\n"
            "Fix <code>users.json</code> or credit by numeric id (not supported here).",
            parse_mode=ParseMode.HTML,
        )
        return
    tid = ids[0]
    try:
        old_b, new_b = admin_credit_usd(tid, amount)
    except ValueError as e:
        await msg.reply_text(escape(str(e)), parse_mode=ParseMode.HTML)
        return
    try:
        await context.bot.send_message(
            chat_id=tid,
            text=(
                f"💵 <b>${amount:.2f}</b> was added to your balance by an admin.\n"
                f"New balance: <b>${new_b:.2f}</b>"
            ),
            parse_mode=ParseMode.HTML,
        )
    except TelegramError as e:
        logger.warning("Could not DM user %s after admin credit: %s", tid, e)
    await msg.reply_text(
        "✅ <b>Credited</b>\n\n"
        f"User: <code>@{escape(un)}</code> · id <code>{tid}</code>\n"
        f"Added: <b>${amount:.2f}</b>\n"
        f"Balance: <code>${old_b:.2f}</code> → <b>${new_b:.2f}</b>",
        parse_mode=ParseMode.HTML,
    )
...
async def touch_user_record(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    users = _load_users()
    uid = str(u.id)
    entry = dict(users.get(uid, {}))
    changed = False
    un = (u.username or "").strip()
    fn = (u.full_name or "").strip()
    if un and entry.get("username") != un:
        entry["username"] = un
        changed = True
    if fn and entry.get("full_name") != fn:
        entry["full_name"] = fn
        changed = True
    if changed:
        users[uid] = {**users.get(uid, {}), **entry}
...
async def on_buy_f_add(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Add filters: handled separately so callback is answered once; qty may be in data (buy_f_add:25)."""
    ...
    ensure_user(user_id)
    data = query.data or ""
    m = re.match(r"^buy_f_add(?::(\d+))?$", data)
    if not m:
        return
    if m.group(1):
        qv = int(m.group(1))
        if qv >= 1:
            context.user_data["buy_qty"] = qv
    qty = int(context.user_data.get("buy_qty") or 0)
    if qty < 1:
        await query.answer(
            "Pick quantity again: tap 🛒 Buy Leads and choose a number.",
            show_alert=True,
        )
        return
    await query.answer()
    ...
    try:
        ...
        logger.warning("buy_f_add edit_message_text: %s", e)
        if query.message:
            await query.message.reply_text(
                buy_filters_grid_text(qty, flt),
                parse_mode=ParseMode.HTML,
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "free_claim":
        ...
        if e.get("free_week") != wk:
            e["free_week"] = wk
            e["free_used"] = 0
        if int(e.get("free_used", 0)) >= FREE_LEADS_PER_WEEK:
            await query.answer(
                "You already claimed this week's free leads.", show_alert=True
            )
            return
        tier = (
            "first"
            if total_line_count("first") >= FREE_LEADS_PER_WEEK
            else "second"
        )
        if total_line_count(tier) < FREE_LEADS_PER_WEEK:
            await query.answer("Not enough stock for free leads.", show_alert=True)
            return
        pairs = pop_n_random_any(FREE_LEADS_PER_WEEK, tier)
        if not pairs or len(pairs) != FREE_LEADS_PER_WEEK:
            await query.answer("Could not pull lines — try again.", show_alert=True)
            return
        e["free_used"] = int(e.get("free_used", 0)) + FREE_LEADS_PER_WEEK
        e["free_week"] = wk
        users[uid_s] = {**users.get(uid_s, {}), **e}
        _save_users(users)
        tlabel = "firsthand" if tier == "first" else "secondhand"
        body = "\n".join(
            f"[{i + 1}] {line}" for i, (_, line) in enumerate(pairs)
        )
        await query.message.reply_document(
            document=InputFile(io.BytesIO(body.encode("utf-8")), filename="free_leads.txt"),
            caption=(
                f"🎁 <b>Free weekly leads</b> · <b>{FREE_LEADS_PER_WEEK}</b> lines "
                f"({tlabel}) · week <code>{escape(wk)}</code>"
            ),
            parse_mode=ParseMode.HTML,
        )
    ...
    if data == "stk:x":
        try:
            await query.edit_message_text("Closed.", parse_mode=ParseMode.HTML)
        except TelegramError:
            pass
        return
    ...
    if data == "stk:mu":
        await query.edit_message_text(
            "<b>Stock by card brand</b>\n\n"
            "<i>Catalog BINs that have lines right now (same idea as sendout).</i>",
            parse_mode=ParseMode.HTML,
            reply_markup=stock_by_brand_menu_markup(),
        )
        return

    m_stk = re.match(r"^stk:v:(visa|mastercard|amex|discover)$", data)
    if m_stk:
        bkey = m_stk.group(1)
        body, nlines = export_stock_lines_for_brand(bkey)
        title = _FILTER_BRAND_LABELS.get(bkey, bkey)
        tit_esc = escape(title)
        if nlines < 1:
            await query.edit_message_text(
                f"🏷️ <b>{tit_esc}</b>\n\n<i>No lines in stock for this network.</i>",
                parse_mode=ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup(
                    [
                        [InlineKeyboardButton("⬅️ Networks", callback_data="stk:mu")],
        ...
        await query.edit_message_text(
            f"🏷️ <b>{tit_esc}</b>\n\n"
            f"<b>{nlines}</b> full line(s) — file in the next message "
            f"(columns: pile, BIN, raw line).",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(
                [
                    [InlineKeyboardButton("⬅️ Networks", callback_data="stk:mu")],
        ...
        await context.bot.send_document(
            chat_id=query.message.chat_id,
            document=InputFile(io.BytesIO(blob), filename=f"stock_{bkey}.tsv"),
            caption=f"🏷️ {tit_esc} — all lines ({nlines}).",
            parse_mode=ParseMode.HTML,
        )
        return
    ...
    if data == "adm_ub":
        await show_admin_user_balances(query, context)
        return
    ...
    if data == "adm_st":
        ...
        blob = export_full_stock_tsv().encode("utf-8")
        await query.message.reply_text(summary)
        await context.bot.send_document(
            chat_id=query.message.chat_id,
            document=InputFile(io.BytesIO(blob), filename="full_stock.tsv"),
            caption=(
                "📊 <b>Full stock</b> — every line in both piles "
                "(pile, BIN, raw line)."
            ),
            parse_mode=ParseMode.HTML,
        )
    ...
    if data == "buy_f_bk":
        ...
        await query.edit_message_text(
            f"<b>Purchase {qty} leads.</b>\n\nAdd filters? <i>(optional)</i>",
            parse_mode=ParseMode.HTML,
            reply_markup=buy_filters_prompt_keyboard(qty if qty >= 1 else None),
        )
        return
    ...
    m_bfp = re.match(r"^bfp:(state|city|zip|bin):(\d+)$", data)
    if m_bfp:
    ...
    m_bfx = re.match(r"^bfx:(state|city|zip|bin):(\d+)$", data)
    if m_bfx:
        dim = m_bfx.group(1)
        ...
        if dim == "bin":
            ...
            zk = norm_zip_filter_value(str(name).strip())
            if zk:
                flt["zip"] = zk
        else:
            flt["city"] = str(name).strip()
    ...
    if data == "bf_acart":
        ...
        if not _has_any_buy_filter(flt):
            await query.answer(
                "Pick Brand, ZIP, State, City, BIN, or Gender first.",
                show_alert=True,
            )
            return
        context.user_data.pop("awaiting_cart_filter_qty", None)
    ...
    m_bfg = re.match(r"^bfg:(male|female|clear)$", data)
    if m_bfg:
        choice = m_bfg.group(1)
        flt = dict(context.user_data.get("buy_filters") or {})
        if choice == "clear":
            flt.pop("gender", None)
        else:
        ...
        if choice == "clear":
            ...
            await query.answer("Gender cleared.")
            return
        fk = _active_buy_filter_kwargs(flt)
        n_first = count_matching_lines("first", **fk)
        n_second = count_matching_lines("second", **fk)
        tot = n_first + n_second
        lbl = "♂ Male" if choice == "male" else "♀ Female"
        await query.edit_message_text(
            f"<b>Gender</b>: {lbl}\n"
            f"Lines: <b>{tot}</b> (1st {n_first} · 2nd {n_second})"
            "\n\n"
    ...
    if data == "bf_gender":
        await query.edit_message_text(
            "⚧️ <b>Gender</b>\n\n<i>Choose male or female.</i>",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("♂ Male", callback_data="bfg:male"),
                        InlineKeyboardButton("♀ Female", callback_data="bfg:female"),
                    ],
                    [InlineKeyboardButton("Clear gender", callback_data="bfg:clear")],
                    [
                        InlineKeyboardButton(
                            "⬅️ Filter menu", callback_data="bf_cancel_prompt"
                        )
    ...
    if data == "rdok":
        ...
        await deliver_purchased_bulk(query, context, pairs, total, ttl)
        await query.edit_message_text(
    ...
    if data == "ca_ok":
        ...
        if not res:
            ...
            return
        pairs, total = res
        await deliver_purchased_bulk(query, context, pairs, total, "Cart checkout")
        await query.edit_message_text(
...
async def _register_bot_menu(app: Application) -> None:
    ...
    default_cmds = [
        ...
        BotCommand(
            "stock", "Browse catalog BINs by Visa / Mastercard / Amex / Discover"
        ),
        BotCommand("request", "Custom lead request"),
        BotCommand("filter", "Open lead filters (after choosing qty)"),
    ...
    admin_cmds = default_cmds + [
        ...
        BotCommand("user", "Lookup user balance by Telegram ID"),
        BotCommand("credit", "Add balance: /credit @username amount"),
    ]
    try:
...
async def _post_init(app: Application) -> None:
    ...
    if jq is None:
        logger.warning(
            "JobQueue unavailable — balance reminders disabled "
            "(install: pip install 'python-telegram-bot[job-queue]')"
        )
        return
    try:
        hours = float((os.environ.get("BALANCE_REMINDER_HOURS") or "15").strip() or 15)
        if hours <= 0:
            hours = 15.0
    except ValueError:
        hours = 15.0
    try:
        first_min = float(
            (os.environ.get("BALANCE_REMINDER_FIRST_MIN") or "3").strip() or 3
        )
        if first_min < 0:
            first_min = 3.0
    except ValueError:
        first_min = 3.0
    jq.run_repeating(
        _scheduled_balance_broadcast,
        interval=timedelta(hours=hours),
        first=timedelta(minutes=first_min),
        name="balance_reminder",
    )
    logger.info(
        "Balance reminders: every %.2f h, first run in %.1f min (env BALANCE_REMINDER_HOURS, "
        "BALANCE_REMINDER_FIRST_MIN).",
        hours,
        first_min,
    )
...
def _run_telegram_polling(token: str) -> None:
    ...
    app.add_handler(CommandHandler("filters", filter_cmd))
    ...
    app.add_handler(
        CallbackQueryHandler(on_buy_f_add, pattern=r"^buy_f_add(?::\d+)?$")
    )
...
def main() -> None:
    ...
    if env_port:
        # Railway / cloud: bind HTTP on $PORT in the main thread first so /health can pass
        # even if TELEGRAM_BOT_TOKEN is missing or mis-set (otherwise deploy never becomes healthy).
        if token:
            ...
            return
        logger.error(
            "TELEGRAM_BOT_TOKEN is not set — HTTP /health is up but the bot will not run. "
            "Add TELEGRAM_BOT_TOKEN in your host's environment variables."
        )
        try:
            from web_upload import run_public_http_forever
